...
#Model
class DrugRec_allCore(nn.Module):
    def __init__(
            self,
            ...
            emb_dim: int = 256,
            num_symptoms: int = 1024,
            device: torch.device = torch.device('cpu:0')):
        super(DrugRec_allCore,self).__init__()
        self.vocab_size=vocab_size
        self.device=device
        self.dim=dim
        self.k_mul=k_mul
        self.multivisit=multivisit
        self.mulhistory=mulhistory
        self.fix_smi_rep=fix_smi_rep
        trans_dim=emb_dim
        self.num_diag=vocab_size[0]
        self.num_proc=vocab_size[1]
        self.num_med=vocab_size[2]
        vocab_dp=max(self.num_diag,self.num_proc)
        self.transformer_dp=Transformer_Encoder(n_layer=2,hidden_size=trans_dim,num_attention_heads=4,
                                                vocab_size=vocab_dp,max_position_size=576,max_segment=40,
                                                word_emb_padding_idx=0,dropout=0.1)

        self.sym_embed=nn.Embedding(num_symptoms,trans_dim,padding_idx=0)
        ...
        if self.fix_smi_rep:
            init_rep=input_smiles_init_rep.to(device=self.device) if isinstance(input_smiles_init_rep,
                                                                                torch.Tensor) else torch.tensor(
                input_smiles_init_rep,dtype=torch.float32).to(device=self.device)
            if vocab_size[2]<=init_rep.shape[0]:
                self.med_rep=init_rep[:vocab_size[2]]
            else:
                existing_rep=init_rep
                extra_count=vocab_size[2]-init_rep.shape[0]
            ...
            init_rep=input_smiles_init_rep.to(device=self.device) if isinstance(input_smiles_init_rep,
                                                                                torch.Tensor) else torch.tensor(
                input_smiles_init_rep,dtype=torch.float32,requires_grad=True).to(device=self.device)
            if vocab_size[2]<=init_rep.shape[0]:
                self.med_rep=init_rep[:vocab_size[2]]
            else:
                existing_rep=init_rep
    ...
    def _encode_symptoms(self,sym_ids_list):
        if not sym_ids_list:
            return torch.zeros(1,1,self.sym_embed.embedding_dim,device=self.device)
        sym_embs=[]
        for sym_ids in sym_ids_list:
            sym_ids=sym_ids.long().squeeze(0)
            if sym_ids.dim()==0:
                sym_ids=sym_ids.unsqueeze(0)
            sym_ids=sym_ids.clamp(min=0,max=self.sym_embed.num_embeddings-1)
            emb=self.sym_embed(sym_ids).mean(dim=0,keepdim=True).unsqueeze(0)
            sym_embs.append(emb)
        return torch.cat(sym_embs,dim=1)

    def forward(self,input,type):
        if not input:
            med_size=self.vocab_size[2]
            return (
                torch.zeros(1,med_size,device=self.device),
                torch.zeros(1,med_size,med_size,device=self.device),
                torch.tensor(0.0,device=self.device),
            )

        DIAG_SEQ,PRO_SEQ,SYM_SEQ=[],[],[]
        trans_dim=self.query[1].in_features//3

        for j,adm in enumerate(input):
            if adm[0]:
                diag_ids_list=[self._to_tensor(diag) for diag in adm[0]]
            else:
                diag_ids_list=[torch.zeros(1,1,dtype=torch.long,device=self.device)]
            diag_representations=self.transformer_dp(diag_ids_list)

            if adm[1]:
                pro_ids_list=[self._to_tensor(pro) for pro in adm[1]]
            else:
                pro_ids_list=[torch.zeros(1,1,dtype=torch.long,device=self.device)]
            pro_representations=self.transformer_dp(pro_ids_list)

            if adm[2]:
                sym_ids_list=[self._to_tensor(sym) for sym in adm[2]]
            else:
                sym_ids_list=[torch.zeros(1,1,dtype=torch.long,device=self.device)]
            sym_representations=self._encode_symptoms(sym_ids_list)
        ...
        sym_seq=self.lin(torch.cat(SYM_SEQ,dim=1))
        ...
        if self.multivisit:
            diag_out,_=self.diag_seq_enc(diag_seq,self.k_mul)
            pro_out,_=self.pro_seq_enc(pro_seq,self.k_mul)
            sym_out,_=self.sym_seq_enc(sym_seq,self.k_mul)
        else:
            diag_out,pro_out,sym_out=diag_seq,pro_seq,sym_seq

        patient_representations=torch.cat([diag_out,pro_out,sym_out],dim=-1).squeeze(dim=0)
...
class DrugRec_all(BaseDrugRecommendationModel):
    def __init__(
            self,
            dim: int,
            vocab_size: Tuple[int,int,int],
        ...
        self.vocab_size=vocab_size
        self.multihistory=multihistory
        self.multivisit=multivisit
        self.k_mul=k_mul
        ...
        self.core=DrugRec_allCore(
            vocab_size=vocab_size,
            fix_smi_rep=fix_smi_rep,
            ddi_adj=ddi_adj,
            multivisit=multivisit,
            mulhistory=multihistory,
    ...
    def forward(self,batch: Dict[str,Any]) -> Dict[str,torch.Tensor]:
        ...
        if not patients or not patients[0] or len(patients[0])==0:
            return {
        ...
        patient=patients[0]
        result,neg_pred_prob,ddi_penalty=self.core(patient,'train')
        return {
            "logits":result,
    ...
    # def _compute_ci_loss(self,outputs,targets):
    #     pred_prob=outputs["pred_prob"]
    #     pred_prob0=outputs["pred_prob0"]
    #     sign_target=torch.sign(targets-0.5)
    #     loss_ci=-torch.log(F.sigmoid(sign_target*(pred_prob-pred_prob0))).mean()
    #     return loss_ci
